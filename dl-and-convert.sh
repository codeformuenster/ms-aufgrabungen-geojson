...
ogr2ogr -nlt POLYGON -f GeoJSON -t_srs crs:84 /vsistdout/ tmp/aufbrueche.shp | jq -r '.["fetch_time"] = "@@date@@" | del(.crs) | tostring' | sed "s/@@date@@/`date --iso-8601=sec`/" > $OUTPUT_DIR/$TODAY.json
